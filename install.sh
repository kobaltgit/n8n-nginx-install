...
### 3. Клонирование проекта с GitHub (ИСПРАВЛЕННАЯ ВЕРСИЯ)
echo "📥 Клонируем проект с GitHub..."

# Переходим в безопасную родительскую директорию /opt
# Если ее нет, создаем
mkdir -p /opt
cd /opt

# Останавливаем старые контейнеры (если они есть) и удаляем старую папку
if [ -d "n8n-install" ]; then
    cd n8n-install
    docker compose down --volumes || true
    cd ..
    echo "Удаляем старую директорию /opt/n8n-install..."
    rm -rf n8n-install
fi

# Клонируем проект заново
echo "Клонируем репозиторий заново..."
git clone https://github.com/kobaltgit/n8n-nginx-install.git n8n-install

# Переходим в новую папку проекта. Если не получилось, выходим с ошибкой.
cd n8n-install || { echo "Не удалось перейти в директорию /opt/n8n-install. Установка прервана."; exit 1; }
